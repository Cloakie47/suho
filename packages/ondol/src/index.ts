...
export interface OndolProviderOptions {
  /** Override the Suho app URL (e.g. http://localhost:5173 for local dev). */
  appUrl?: string;
}
...
export interface OndolProvider {
  request(args: RequestArguments): Promise<unknown>;
  on(event: string, listener: Listener): void;
  removeListener(event: string, listener: Listener): void;
  /** True while a popup request is open. */
  readonly isSuho: true;
}
 ...
 * ```
 */
export function createOndolProvider(options: OndolProviderOptions = {}): OndolProvider {
  ...
  function popupRequest(method: string, params: unknown[]): Promise<unknown> {
    if (inflight) {
    ...
    return new Promise<unknown>((resolve, reject) => {
      ...
      const onMessage = (e: MessageEvent) => {
        if (e.origin !== appOrigin) return; // only the Suho app origin
        if (e.source !== popup) return; // only the window we opened
        const d = e.data as { __suho?: string; v?: number; id?: string; result?: unknown; error?: { code?: number; message?: string } };
        ...
        if (d.__suho === "res" && d.id === id) {
          if (d.error) finish(() => reject(new OndolRpcError(d.error?.code ?? 4001, d.error?.message ?? "Request failed.")));
          else finish(() => resolve(d.result));
          try {
            popup.close();
          } catch {
            /* already closed */
      ...
      const closeTimer = setInterval(() => {
        if (popup.closed) finish(() => reject(new OndolRpcError(4001, "User closed the window.")));
      }, 400);
  ...
  async function request(args: RequestArguments): Promise<unknown> {
    ...
    switch (method) {
      ...
      case "eth_requestAccounts": {
        const result = (await popupRequest(method, params)) as string[];
 ...
 */
export async function verifyOndolSignature(
  hash: string,
  signature: string,
  account: string,
  rpcUrl: string,
